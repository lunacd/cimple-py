...
RUN `
    ...
    $p = Start-Process -FilePath 'C:\TEMP\vs_buildtools.exe' -Wait -PassThru `
        -ArgumentList '--quiet', '--wait', '--norestart', '--nocache', '--includeRecommended', '--add', 'Microsoft.VisualStudio.Workload.VCTools'; `
...
RUN $null = [Environment]::SetEnvironmentVariable('VSINSTALLDIR', 'C:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools', [EnvironmentVariableTarget]::Machine)
